...
# 检查是否已经打过补丁（使用新的标记）
if '# PATCHED_BY_WRAPPER_V3' in original_content:
...
def _patched_create(self, tx, add_tags=None, **kwargs) -> str:
    """使用交易日期生成文件路径"""
    # 提取交易日期
    if hasattr(tx, 'date'):
        # 是 Transaction 对象
    ...
    elif hasattr(tx, 'meta') and 'date' in tx.meta:
        # 从 meta 中提取日期
        date_str = tx.meta['date']
    ...
    # 使用交易日期格式化路径
    # 修改私有属性 __bean_file (Python name mangling: _TransactionManager__bean_file)
    formatted_path = self._TransactionManager__bean_file.format(
        year=str(year),
        month=f"{month:02d}",
    ...
    # 临时修改路径
    original_path = self._TransactionManager__bean_file
    self._TransactionManager__bean_file = formatted_path
    
    try:
        result = _original_create(self, tx, add_tags, **kwargs)
        print(f"[PATCH] Saved to: {formatted_path} (date: {year}-{month:02d})")
        return result
    finally:
        self._TransactionManager__bean_file = original_path
